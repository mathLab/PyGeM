...
It performs Free Form Deformation while trying to enforce a costraint of
the form F(x)=c. The constraint is enforced exactly (up to numerical
errors) if and only if the function is linear. For details on Free Form
Deformation see the mother class.
"""
...
class CFFD(FFD):
    """Class that handles the Constrained Free Form Deformation on the mesh
    points.

    :param list n_control_points: number of control points in the x, y, and z
        direction. Default is [2, 2, 2].
    :param string mode: it can be ``affine`` or ``triaffine``. The first option
      is for the F that are affine in all the coordinates of the points.
        The second one is for functions that are F in the coordinates of the
          points. The first option implies the second, but is optimal for
    ...
    :cvar numpy.ndarray fixval: it defines the c of the
      constraint F(x)=c. Default is 1.
    :cvar numpy.ndarray ffd_mask: a boolean tensor that tells to the class
        ...
        The tensor has shape (n_x,n_y,n_z,3), where the last dimension
          indicates movement
        on x,y,z respectively. Default is all true.
    :cvar numpy.ndarray fun_mask: a boolean tensor that tells to the class
        on which axis which constraint depends on. The tensor has shape
          (n_cons,3), where the last dimension indicates dependency
            on x,y,z respectively. Default is all true.
              It used only in the triaffine mode.
    ...
    :Example:
        ...
        >>> assert np.isclose(np.linalg.norm(fun(cffd.ffd(original_mesh_points
        ... [:-4])) - b), np.array([0.]), atol = 1e-06)
        >>> new_mesh_points = cffd.ffd(original_mesh_points)
    """

    def __init__(
        self, fixval, fun, n_control_points=None, ffd_mask=None, fun_mask=None
    ):
        super().__init__(n_control_points)

        if ffd_mask is None:
            self.ffd_mask = np.full(
                (*self.n_control_points, 3), True, dtype=bool
            )
        else:
    ...
    def adjust_control_points(self, src_pts):
        ...
        :param np.ndarray src_pts: the points whose deformation we want
            to be constrained.
        :rtype: None.
        """
        ...
        for i in range(3):
            self.ffd_mask = np.full(
                (*self.n_control_points, 3), False, dtype=bool
            )
...
# pylint: disable=C0103
    ...
    def _save_parameters(self):
        ...
        :return: the FFD control points in an array of shape
            [n_x,ny,nz,3].
        :rtype: numpy.ndarray
        """
    ...
    def _compute_linear_map(self, src_pts, saved_parameters, indices):
        """Computes the coefficient and the intercept of the linear map from
        the control points to the output.

        :param np.ndarray src_pts: the points to deform.
        :param np.ndarray saved_parameters: the array of FFD control
            points.
        ...
        for i in range(
            ...
            def_pts = super().__call__(
                src_pts
            )  # computing the deformation with the dependent variable
            outputs[i] = self.fun(def_pts)  # computing the independent variable
        sol = np.linalg.lstsq(
            inputs, outputs, rcond=None
        )  # computation of the linear map
        A = sol[0].T[:, :-1]  # coefficient pylint: disable=C0103
        b = sol[0].T[:, -1]  # intercept pylint: disable=C0103
        return A, b
