...
class VFFD(CFFD):
    """Class that handles the Volumetric Free Form Deformation on mesh points.
...
:param string mode: it can be ``affine`` or ``triaffine``. The first option
    ...
    points. The first option implies the second, but is optimal for that
    class of functions.
...
:cvar callable fun: it defines the F of the constraint F(x)=c. Default is
...
:cvar numpy.ndarray ffd_mask: a boolean tensor that tells to the class
    which control points can be moved, and in what direction, to enforce
    the constraint. The tensor has shape (n_x,n_y,n_z,3), where the last
    dimension indicates movement on x,y,z respectively. Default is all
    true.
:cvar numpy.ndarray fun_mask: a boolean tensor that tells to the class
    ...
    (n_cons,3), where the last dimension indicates dependency on x,y,z
    respectively. Default is all true. It used only in the triaffine mode.
...
:Example:
    ...
    >>> assert np.isclose(np.linalg.norm(vffd.fun(new_mesh_points) - b),
    ...                   np.array([0.]), atol=1e-07)
"""
