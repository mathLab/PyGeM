...
ffd.array_mu_y = ffd.array_mu_y + 0.5 * np.random.rand(*ffd.array_mu_x.shape)
ffd.array_mu_z = ffd.array_mu_z + 0.5 * np.random.rand(*ffd.array_mu_x.shape)
x_def = ffd(x)
x_def = x_def
...
print("The custom linear function on the non deformed points is",
      custom_linear_constraint(x))
print("The custom linear function on the classic FFD deformed points is",
      custom_linear_constraint(x_def))
from pygem.cffd import CFFD
ffd = CFFD(np.array([1.]), custom_linear_constraint, [2, 2, 2])
np.random.seed(0)
ffd.array_mu_x = ffd.array_mu_x + 0.0 * np.random.rand(*ffd.array_mu_x.shape)
ffd.array_mu_y = ffd.array_mu_x + 0.5 * np.random.rand(*ffd.array_mu_x.shape)
ffd.adjust_control_points(x)
x_def = ffd(x)
plt.plot(x_def[:, 0], x_def[:, 1], 'o')
print("The custom linear function on the constrained FFD deformed points is",
      custom_linear_constraint(x_def))
...
vffd = VFFD(faces,initvolume,[2, 2, 2])
np.random.seed(0)
...
print("Percentage difference from the original mesh is ",
      np.linalg.norm(mesh_def - points) / np.linalg.norm(points) * 100)
fig = plt.figure(figsize=plt.figaspect(0.5))
ax = fig.add_subplot(1, 2, 1, projection='3d')
ax.plot_trisurf(mesh_def[:, 0],
                mesh_def[:, 1],
                mesh_def[:, 2],
                triangles=faces,
                cmap=plt.cm.Spectral)
