...
class TestElmerHandler(TestCase):
    ...
    def test_elmer_parse_failing_filename_type(self):
        ...
        with self.assertRaises(TypeError):
            mesh_points = elmer_handler.parse(0.2)

    def test_elmer_parse_failing_check_extension(self):
        elmer_handler = elh.ElmerHandler()
        with self.assertRaises(ValueError):
            mesh_points = elmer_handler.parse("tests/test_datasets/test_square.iges")

    def test_elmer_parse_infile(self):
        elmer_handler = elh.ElmerHandler()
        mesh_points = elmer_handler.parse("tests/test_datasets/test_elmer.nodes")
        self.assertEqual(elmer_handler.infile, "tests/test_datasets/test_elmer.nodes")
    ...
    def test_elmer_write_failing_check_extension(self):
        ...
        mesh_points = elmer_handler.parse("tests/test_datasets/test_elmer.nodes")
        with self.assertRaises(ValueError):
            elmer_handler.write(mesh_points, "tests/test_datasets/test_square.iges")
